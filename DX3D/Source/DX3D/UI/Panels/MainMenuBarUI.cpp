...
void MainMenuBarUI::render(const Callbacks& callbacks) {
    if (ImGui::BeginMainMenuBar()) {
        ...
        if (ImGui::BeginMenu("Edit")) {
            ...
            if (!canUndo || !isEditMode) ImGui::PushStyleVar(ImGuiStyleVar_Alpha, 0.6f);
            if (ImGui::MenuItem("Undo", "Ctrl+Z", false, canUndo && isEditMode)) m_controller.onUndoClicked();
            if (!canUndo || !isEditMode) ImGui::PopStyleVar();
            if (canUndo && isEditMode) {
                ImGui::SameLine();
                ImGui::TextDisabled("(%s)", m_undoRedoSystem.getUndoDescription().c_str());
            }
            if (!canRedo || !isEditMode) ImGui::PushStyleVar(ImGuiStyleVar_Alpha, 0.6f);
            if (ImGui::MenuItem("Redo", "Ctrl+Shift+Z", false, canRedo && isEditMode)) m_controller.onRedoClicked();
            if (!canRedo || !isEditMode) ImGui::PopStyleVar();
            ...
            if (!hasSelection || !isEditMode) ImGui::PushStyleVar(ImGuiStyleVar_Alpha, 0.6f);
            if (ImGui::MenuItem("Delete", "Delete", false, hasSelection && isEditMode)) m_controller.onDeleteClicked();
            if (!hasSelection || !isEditMode) ImGui::PopStyleVar();
            ImGui::EndMenu();
        }
