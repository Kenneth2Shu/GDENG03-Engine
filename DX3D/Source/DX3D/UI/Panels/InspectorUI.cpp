...
void InspectorUI::renderLight(std::shared_ptr<LightObject> lightObject) {
    ...
    if (ImGui::Combo("Type", &currentType, lightTypes, 3)) lightData.type = currentType;

    ImGui::ColorEdit3("Color", &lightData.color.x);
    ImGui::DragFloat("Intensity", &lightData.intensity, 0.1f, 0.0f, 10.0f);

    if (lightData.type == LIGHT_TYPE_POINT || lightData.type == LIGHT_TYPE_SPOT) ImGui::DragFloat("Radius", &lightData.radius, 1.0f, 1.0f, 100.0f);
    ...
    if (lightData.type == LIGHT_TYPE_SPOT) {
        ...
        if (ImGui::SliderFloat("Inner Angle", &innerAngle, 0.0f, 90.0f)) lightData.spot_angle_inner = innerAngle * 3.14159f / 180.0f;
        if (ImGui::SliderFloat("Outer Angle", &outerAngle, 0.0f, 90.0f)) lightData.spot_angle_outer = outerAngle * 3.14159f / 180.0f;
...
void InspectorUI::renderCamera(std::shared_ptr<AGameObject> object) {
    ...
    float fov = camera->getFOV() * 180.0f / 3.14159f;
    if (ImGui::SliderFloat("FOV", &fov, 30.0f, 120.0f)) camera->setFOV(fov * 3.14159f / 180.0f);

    float nearPlane = camera->getNearPlane();
    if (ImGui::DragFloat("Near Plane", &nearPlane, 0.01f, 0.01f, 10.0f)) camera->setNearPlane(nearPlane);

    float farPlane = camera->getFarPlane();
    if (ImGui::DragFloat("Far Plane", &farPlane, 1.0f, 10.0f, 1000.0f)) camera->setFarPlane(farPlane);
}
...
void InspectorUI::renderPhysics(std::shared_ptr<AGameObject> object) {
    ...
    if (physicsComp->bodyType == PhysicsBodyType::Dynamic) {
        ...
        if (ImGui::DragFloat("Mass", &mass, 0.1f, 0.1f, 100.0f)) object->setPhysicsMass(mass);
        Vector3 velocity = object->getLinearVelocity();
        ImGui::Text("Velocity: (%.2f, %.2f, %.2f)", velocity.x, velocity.y, velocity.z);
    }

    float restitution = physicsComp->restitution;
    if (ImGui::SliderFloat("Restitution", &restitution, 0.0f, 1.0f)) object->setPhysicsRestitution(restitution);

    float friction = physicsComp->friction;
    if (ImGui::SliderFloat("Friction", &friction, 0.0f, 1.0f)) object->setPhysicsFriction(friction);
}
